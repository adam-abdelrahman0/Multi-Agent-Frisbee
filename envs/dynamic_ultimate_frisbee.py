...
class DynamicUltimateFrisbeeEnv(ParallelEnv):
    """
    Optimized PettingZoo ParallelEnv for ultimate frisbee.

    Key features:
    - Seeded RNG for determinism.
    - Normalized observations [0,1]; stable ordering.
    - Defender assignment via Hungarian algorithm (fallback to greedy if SciPy missing).
    - Smooth float-based movement (no integer snapping or occupancy sets).
    - Shaping rewards: +1 score, +0.05 completed pass, -0.1 turnover/intercept,
      +0.001 * progress toward scoring endzone while team_0 has possession.
    - Small defaults (2v2 on 10x10) to ease initial learning.
    """
    metadata = {"render_modes": ["human"], "name": "ultimate_frisbee_v1"}
    ...
    def __init__(
        ...
        if use_regulation_field:
            ...
            self.field_size = (self.playing_length + 2 * self.endzone_depth, self.playing_width)  # (110,40)
        else:
        ...
        self.action_spaces = {a: spaces.Discrete(5) for a in self.agents}  # 0:up 1:down 2:left 3:right 4:throw

        # Parameters
        self.intercept_range = 1.0
        self.catch_range = 3.0
        self.defender_mark_distance = 2.0
        self.possession_stall_limit = 200
    ...
    # ---- PettingZoo niceties (silence warnings) ----
    ...
    # ---- Utils ----
    def _team_of(self, agent: str) -> int:
        return int(agent.split("_")[1])

    def _obs_dim(self) -> int:
        t = self.num_players_per_team
        # self(2) + disc(2) + flag(1) + teammates(2*(t-1)) + opponents(2*t)
        return 2 + 2 + 1 + 2 * (t - 1) + 2 * t

    def _norm(self, xy: np.ndarray) -> np.ndarray:
        w, h = self.field_size
    ...
    def reset(self, seed=None, options=None):
        ...
        self._pending_thrower = None
        self._prev_dist_to_goal = None  # for progress shaping
    ...
    def step(self, actions):
        ...
        # shaping: progress toward offense endzone (team_0 → right side)
        if (not self.disc_in_flight) and (self.possession is not None) and (self._team_of(self.possession) == 0):
            ...
            if self._prev_dist_to_goal is not None:
                progress = self._prev_dist_to_goal - dist_to_goal  # positive if moved right
                rewards[self.possession] += 0.001 * progress
            self._prev_dist_to_goal = dist_to_goal
        else:
            self._prev_dist_to_goal = None  # freeze progress until offense regains steady possession
        ...
        if poss_team == 0 and px >= (self.endzone_depth + self.playing_length):
            ...
            terminations = {a: True for a in self.agents}
        elif poss_team == 1 and px < self.endzone_depth:
            rewards[self.possession] += 1.0
            terminations = {a: True for a in self.agents}
    ...
    def _throw_disc(self, agent):
        ...
        # bias toward open teammate
        openness = []
        for t in teammates:
            tpos = self.agent_positions[t]
            nearest_def = min(
                np.linalg.norm(self.agent_positions[d] - tpos)
                for d in self.agents if self._team_of(d) == 1 - team
            )
            openness.append(nearest_def)
        probs = np.array(openness, dtype=float)
    ...
    def _update_disc_flight(self, rewards: dict) -> bool:
        ...
        if self.disc_target is not None:
            ...
            if np.linalg.norm(recv_pos - self.disc_position) <= self.catch_range:
                ...
                self.possession_timer = 0
                self._pending_thrower = None
                # reset progress shaping base on next step
                self._prev_dist_to_goal = None
                return False
        ...
        # out of bounds → turnover to nearest defender
        x_hit = (self.disc_position[0] <= 0) or (self.disc_position[0] >= self.field_size[0] - 1)
        y_hit = (self.disc_position[1] <= 0) or (self.disc_position[1] >= self.field_size[1] - 1)
        if x_hit or y_hit:
            if self._pending_thrower:
                rewards[self._pending_thrower] -= 0.1
    ...
    def _turnover_on_stall(self):
        ...
        defenders = [a for a in self.agents if self._team_of(a) == 1 - old_team]
        if defenders:
        ...
        if _HAS_SCIPY:
            ...
            for d in sorted(defenders, key=lambda dd: np.linalg.norm(self.agent_positions[dd] - self.disc_position)):
                if offs_remaining:
                    nearest = min(
                        offs_remaining,
                        key=lambda o: np.linalg.norm(self.agent_positions[d] - self.agent_positions[o]),
                    )
                    marks[d] = nearest
    ...
    def render_matplotlib(self, block=False):
        ...
        for d, mark in self.defensive_marks.items():
            if mark is not None:
                ...
                ax.plot([dpos[0], mpos[0]], [dpos[1], mpos[1]], linestyle="--", linewidth=1.0, alpha=0.5, color="gray")

        if self.disc_in_flight and self.disc_target is not None:
            target_pos = self.agent_positions[self.disc_target]
            ax.plot([self.disc_position[0], target_pos[0]], [self.disc_position[1], target_pos[1]],
